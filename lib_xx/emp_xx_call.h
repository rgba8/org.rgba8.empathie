...
EMP_INLINE void error_hook(void)
{
    return;
}
} }
...
#define EMP_RET_IF_X(x_Test, ...)\
    EMP_PP_CAT(EMP_RET_X_IF_X_, EMP_PP_BOOL(EMP_PP_EQUAL(1, EMP_VAARGS_COUNT(__VA_ARGS__))))(x_Test, EMP_PP_EMPTY(), __VA_ARGS__)

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define EMP_RET_IF(           ...)  EMP_RET_IF_X( == true  , __VA_ARGS__ )
#define EMP_RET_IF_NOT(       ...)  EMP_RET_IF_X( == false , __VA_ARGS__ )
